...
removePositionBiais <- function(data, col){
  ...
  postPrePosEffects_x <- extendFit(postPrePosEffects)
  postPrePosEffects_ci <- postPrePred$se.fit * qt(0.95 / 2 + .5, postPrePred$df)
  postPrePosEffects_ci_ymin <- postPrePosEffects - postPrePosEffects_ci
  postPrePosEffects_ci_ymax <- postPrePosEffects + postPrePosEffects_ci
  postPrePosEffects.df <- data.frame(pos = posseq, 
                                        postPrePosEffects, 
                                     postPrePosEffects_x, 
                                        postPrePosEffects_ci_ymin, 
                                        postPrePosEffects_ci_ymax,
                                        postPrePred_se = postPrePred$se.fit)
  postPrePosEffectsData <- merge(data, postPrePosEffects.df[, c("pos", "postPrePosEffects_x")], by = "pos", all.x = TRUE)
  return(log2(postPrePosEffectsData[, c(col)]) - postPrePosEffectsData$postPrePosEffects_x)
}
